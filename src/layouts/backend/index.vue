<template>
    <component :is="safeLayoutMode"></component>
</template>
...
// 有效的布局模式列表
const validLayoutModes = ['Default', 'Classic', 'Streamline', 'Double']
...
// 安全的布局模式，如果配置的模式无效则回退到 Default
const safeLayoutMode = computed(() => {
    const mode = config.layout.layoutMode
    if (validLayoutModes.includes(mode)) {
        return mode
    }
    // 无效的布局模式，重置为 Default
    console.warn(`无效的布局模式: ${mode}，已回退到 Default`)
