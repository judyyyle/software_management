<template>
    <el-dialog
        ...
        :title="baTable.form.operate === 'Add' ? '新增设备' : '编辑设备'"
        width="800px"
    ...
    >
        ...
        >
            <div class="flex device-row">
                ...
                <el-form-item label="设备标识(用于后续区分您的设备)" prop="project_id" required class="flex-1">
            ...
            <div class="flex device-row">
                <el-form-item label="设备SN码" prop="sn" required class="flex-1">
                    ...
                    <CustomInput v-model="baTable.form.items!.sn" placeholder="请输入设备SN码" width="100%"> </CustomInput>
                </el-form-item>
...
const handleSN = () => {
    ...
    setTimeout(() => {
        console.log('校验的sn码数据', deviceOsds.value[sn])
        loading.value = false
        if (Object.keys(deviceOsds.value[sn]).length) {
            isValidateSN.value = true
            ElMessage.success('SN码效验成功')
        } else {
