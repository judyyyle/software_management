<template>
    <div class="equipment-box">
        ...
        <Table ref="tableRef">
            ...
            <template #mode_code>
                <el-table-column :label="t('equipment.mode_code')">
                    <template #default="scope">
                        <el-tag v-if="Object.keys(deviceOsds[scope.row['sn']]).length > 0">{{
                            t(`equipment.mode_code ${deviceOsds[scope.row['sn']].mode_code}`)
                        }}</el-tag>
                        <el-tag type="danger" v-else>设备离线</el-tag>
                    </template>
                </el-table-column>
            </template>
        </Table>
...
let newButton: OptButton[] = [
    ...
    },
    {
        ...
        click: async (row: TableRow, field: TableColumn) => {
            if (Object.keys(deviceOsds.value[row.sn]).length == 0) {
                return ElMessage.warning('设备不在线')
            }
