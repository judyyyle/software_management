<template>
    <div class="ship-lanes-options">
        <div class="options-header">
            <div class="options-header-title">航线基本配置功能</div>
        ...
        <div class="options-content flex-col">
            ...
            <div class="content-item flex-col">
                ...
                <div class="flex justify-between">
                    <div class="speed-item flex align-center justify-center" @click="changeSpeed(-1)">
                        <el-icon><Minus /></el-icon>
                    </div>
                    <div class="speed-input flex-1 flex align-center justify-center">
                        <el-input type="number" v-model="form.Folder.autoFlightSpeed" class="flex-1" :min="1" :max="15" @input="inputSpeed" />
...
const saveShipLanes = async () => {
    // 如果速度超过15m/s，提示用户
    if (Number(form.value.Folder.autoFlightSpeed) > 15) return ElMessage.error('速度不能超过15m/s')
...
const changeSpeed = (val: number) => {
    if (Number(form.value.Folder.autoFlightSpeed) + val > 15) return ElMessage.error('速度不能超过15m/s')
    if (Number(form.value.Folder.autoFlightSpeed) + val < 1) return ElMessage.error('速度不能小于1m/s')
    form.value.Folder.autoFlightSpeed = Number(form.value.Folder.autoFlightSpeed) + val
}

// speed输入框
const inputSpeed = (val: string) => {
    if (Number(val) > 15) {
        form.value.Folder.autoFlightSpeed = 15
        return ElMessage.error('速度不能超过15m/s')
    }
    if (Number(val) < 1) {
        form.value.Folder.autoFlightSpeed = 1
        return ElMessage.error('速度不能小于0m/s')
    }
}
