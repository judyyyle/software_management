...
const { subForm, placemarkData, getCurrentPlacemarkAction } = storeToRefs(shipLanesStore)
...
watch(
    ...
    (newVal) => {
        ...
        placemarkData.value.forEach((item) => {
            if (item.useGlobalHeight == 1) {
                item.executeHeight = newVal
            }
        })
        shipLanesStore.changeHeightModel()
    }
...
watch(activeShipLanes, (newVal: string) => {
    if (newVal) {
        console.log(shipStartPoint.value[0])
...
const addStartPoint = (cartesian: any) => {
    ...
    // 添加航点
    shipLanesStore.addPlacemark(point.lng + ',' + point.lat, true, point.alt)
}
...
const addWaypoint = (cartesian: any) => {
    ...
    const point = mars3d.LngLatPoint.fromCartesian(cartesian)
    console.log(point.lng, point.lat, point.alt)
    ...
    // 添加航点
    shipLanesStore.addPlacemark(point.lng + ',' + point.lat, false, point.alt)
}

// 添加模型
const addModel = (index: number) => {
    const models = graphicLayer_m.value.getGraphics()
    console.log(models)
    let heading = 180
    if (models.length > 0) {
        if (placemarkData.value[index - 1]) {
            let data = placemarkData.value[index - 1].actionGroup.action.find((item: any) => item.type === 'rotateYaw')
            if (data) {
                heading = Number(data.actionActuatorFuncParam?.aircraftHeading) + 180 || 180
            }
        }
        if (index === 0) {
            models.forEach((item: any) => {
                item.position = newStartPoint.value[index]
            })
        } else {
            models.forEach((item: any) => {
                item.position = newShipLanes.value[index - 1]
            })
        }
        models.forEach((item: any) => {
            item.attr.index = index
            if (item.id === 'frustum') {
                item.setStyle({ heading })
            }
        })
        return
    }
    ...
    graphicLayer_m.value.addGraphic(graphicFrustum)
    graphicList.value.push(graphicModel)
    graphicList.value.push(graphicFrustum)
}
    ...
    if (models.length > 0) {
        if (index === 0) {
            ...
            models.forEach(async (item: any) => {
                ...
                if (item.attr.type === 5) {
                    let heading = 180
                    if (getCurrentPlacemarkAction.value) {
                        let data = getCurrentPlacemarkAction.value.find((item: any) => item.actionActuatorFunc === 'rotateYaw')
                        if (data) {
                            heading = Number(data.actionActuatorFuncParam?.aircraftHeading) + 180 || 180
                        }
                    }
