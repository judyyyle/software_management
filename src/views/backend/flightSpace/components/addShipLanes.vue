        ...
        <div class="content">
            ...
            <div class="content-item">
                ...
                <el-radio-group v-model="form.flightType" style="width: 100%">
                    <div class="content-item-content">
                        <div
                            class="content-item-content-box"
                            :class="{ active: form.flightType === index }"
                            v-for="(item, index) in source"
                            :key="index"
                        >
                            <el-radio id="radio" :value="index" size="large" style="width: 100%; height: 100%">{{ item.series_name }}</el-radio>
                        </div>
                    </div>
                </el-radio-group>
            </div>
            <div class="content-item">
                ...
                <el-radio-group v-model="form.flightModel" style="width: 100%" v-if="source[form.flightType]">
                    <div class="content-item-content">
                        <div
                            class="content-item-content-box"
                            :class="{ active: form.flightModel === index }"
                            v-for="(item, index) in source[form.flightType].aircraft"
                            :key="index"
                        >
                            <el-radio id="radio" :value="index" size="large" style="width: 100%; height: 100%">{{ item.label }}</el-radio>
                        </div>
                    </div>
                </el-radio-group>
            </div>
...
import { defineModel, inject, ref, Ref, watch, reactive } from 'vue'
...
const handleConfirm = () => {
    subForm.value.drone_model_key = source.value[form.flightType].aircraft[form.flightModel].value
...
const getFlightList = async () => {
    try {
        const res = await createAxios({
            url: '/admin/Equipment/getAircraftList',
            method: 'get',
        })
        source.value = res.data.series || []
        console.log(res)
    } catch (error) {
        console.log(error)
    }
