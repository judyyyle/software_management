<template>
    <div class="right-media-library" v-loading="loading">
        <div class="header">
            <div
                class="header-item"
                    ...
                    <div class="breadcrumb">
                        <div class="breadcrumb-item">
                            <el-button
                                ...
                                type="primary"
                                text
                                @click="handleBreadcrumbClick(breadcrumbList[0])"
                                v-if="breadcrumbList.length > 1"
                                >返回</el-button
                            >
                            <span>{{ breadcrumbList[0] }}</span>
                            <span class="breadcrumb-item-separator" v-if="breadcrumbList.length > 1"> / </span>
                            <span v-if="breadcrumbList.length > 1">{{ breadcrumbList[1] }}</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="content-list" v-if="!mediaFileList.length">
                ...
                >
                    ...
                    <div class="content-item-title">
                        <div class="content-item-title-name">{{ item.name || '无' }}</div>
                    </div>
                </div>
            </div>
            ...
            <el-table :data="mediaFileList" style="width: 100%" v-else>
                ...
                <el-table-column prop="object_key" label="图片" align="center">
                    <template #default="scope">
                        <el-image
                            class="table-img"
                            :src="baseUrl + scope.row.object_key"
                            v-if="scope.row.isImg"
                            @click="handlePreview(baseUrl + scope.row.object_key)"
                        />
                        <video class="table-img" :src="baseUrl + scope.row.object_key" v-else-if="scope.row.isVideo" controls></video>
                        <div v-else></div>
                    </template>
                </el-table-column>
        ...
        <el-image-viewer :scale="0.7" v-if="showPreview" :url-list="srcList" show-progress @close="showPreview = false" />
    </div>
</template>

<script setup lang="ts">
import { ref, reactive, onMounted, watch, useTemplateRef, provide } from 'vue'
import { Search, Download, Delete, ArrowLeft } from '@element-plus/icons-vue'
...
const getMediaFile = async (bid: string | number) => {
    ...
    // 截取文件后缀
    mediaFileList.value.forEach((item: any) => {
        item.src_type = item.object_key.split('.').pop()
        // 判断是图片还是视频
        if (item.src_type === 'jpg' || item.src_type === 'png' || item.src_type === 'jpeg') {
            item.isImg = true
        } else if (item.src_type === 'mp4' || item.src_type === 'avi' || item.src_type === 'mov') {
            item.isVideo = true
        }
    })
    console.log(mediaFileList.value)
    total.value = res.data.total
    console.log(flightTaskList.value[currentShipLane.value])
    if (mediaFileList.value.length > 0 && !breadcrumbList.value.includes(flightTaskList.value[currentShipLane.value].name)) {
        breadcrumbList.value.push(flightTaskList.value[currentShipLane.value].name)
    }
}
