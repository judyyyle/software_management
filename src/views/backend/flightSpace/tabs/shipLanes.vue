<template>
    <div class="shipLanes">
        ...
        <div class="shipLanes-content">
                        ...
                        <div :class="['title-wrapper', { 'is-active': isActive }]">
                            <div class="title-wrapper-left">
                                ...
                                <el-tooltip class="box-item" effect="dark" :content="item.name" placement="top">
                                    <span class="left-span">{{ item.name }}</span>
                                </el-tooltip>
                            </div>
                            <div class="title-wrapper-right">
                                ...
                                <!-- 重命名 -->
                                <el-tooltip class="box-item" effect="dark" content="重命名" placement="top">
                                    <el-icon @click.stop="handleRenameK(item)" size="14" style="cursor: pointer"><EditPen /></el-icon>
                                </el-tooltip>
                                <!-- 新建 -->
                    ...
                    <div class="shipLanes-content">
                        ...
                        >
                            ...
                            <div class="item-content">
                                ...
                                <div class="item-box-text">{{ item1.drone_model_name }}</div>
                            </div>
                            <div class="item-content">
            ...
            <el-empty v-if="shipLanesFolderList.length === 0" description="暂无航线库">
                <el-button type="primary" @click="openAddShipLanePop = true">添加航线库</el-button>
            </el-empty>
        </div>
...
const cockpitStore = useCockpit()
const { currentLane } = storeToRefs(cockpitStore)
...
const handleActive = (item: any, index: string) => {
    console.log(JSON.parse(item.kmz_json))
...
const handleEdit = (item: any, index: string, item1: any) => {
    ...
    setTimeout(() => {
        ...
        subForm.value.project_id = currentProject.value?.id
        subForm.value.drone_model_key = item.drone_model_key
        const kmzJson = JSON.parse(item.kmz_json)
        shipLanesStore.showShipForm(kmzJson)
...
// 重命名航线库
const handleRenameK = (item: any) => {
    ElMessageBox.prompt('航线库名称', '编辑航线库名称', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        inputErrorMessage: '输入的航线库名称不能为空',
        ...
        item.name = value
        let res = await new baTableApi('/admin/airline.Floder/').editPost(item)
        if (res.code == 1) {
