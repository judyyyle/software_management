...
const message_services = computed(() => mqttStore.getMessagesByTopic(`thing/product/1581F6QAD247R00GG53P/state`))
...
const handleEdit = (item: any, index: number) => {
    if (Object.keys(deviceOsds.value[item.sn]).length == 0) {
        return ElMessage.warning('设备离线')
    }

    if (currentEditIndex.value === index) {
        // 关闭订阅
        DJIoperations.deviceServicesReplyClose(gateway_sn.value)
        ...
        // 订阅飞行器状态
        DJIoperations.deviceServicesReply('1581F6QAD247R00GG53P')
    }
}
...
const addMapPoint = (item: any) => {
    const graphic = new mars3d.graphic.DivGraphic({
        ...
        style: {
            html: `<img src="/img/jicang-icon.png" alt="icon" style="width: 60px; height: 60px;" />`,
...
const getDroneStatus = (sn: string) => {
    if (!deviceOsds.value[sn] || Object.keys(deviceOsds.value[sn]).length == 0) {
        return '飞行器离线'
    }
    if (deviceOsds.value[sn].sub_device) {
        const droneSn = deviceOsds.value[sn].sub_device.device_sn
        const droneData = droneOsds.value[droneSn]
        if (deviceOsds.value[sn].sub_device.device_online_status === 1 && droneData?.mode_code !== undefined) {
            return drone_status[droneData.mode_code] || '未知状态'
        } else if (deviceOsds.value[sn].sub_device.device_online_status === 1 && !droneData?.mode_code) {
            return '舱内开机'
        } else {
            return '舱内关机'
        }
...
onMounted(async () => {
    ...
    // 发送消息
    DJIoperations.deviceStatus('7CTXN3V00B08N2')
})
</script>
