...
const { rotatePoint } = storeToRefs(mapStore)
...
// 进入项目
const goProject = async (item: any) => {
    ...
    const rotate = projectRotate.find((rotateItem: any) => rotateItem.id === item.id)?.rotate
    if (rotate == 1) {
...
// 提示
const msgProject = () => {
    if (addProjectPopType.value === 'add') {
        return ElMessage.info('当前正在添加项目')
    }
    return ElMessage.info('当前正在编辑项目')
}
