<template>
    <div class="add-ship">
        <div class="add-ship-header">
            <div class="add-ship-header-title">{{ addProjectPopType == 'add' ? '添加项目' : '编辑项目' }}</div>
        ...
        <el-form ref="formRef" :model="form" :rules="rules" label-width="80px" class="add-ship-form">
            ...
            <el-form-item label="绕点旋转" prop="rotate">
                <el-switch v-model="form.rotate" :active-value="1" :inactive-value="0" active-text="是" inactive-text="否" />
            </el-form-item>
...
// 项目绕点旋转
const projectRotate = JSON.parse(localStorage.getItem('projectRotate') || '[]')
...
onMounted(() => {
    ...
    // 检查项目是否存在绕点旋转
    if (projectRotate.find((item: any) => item.id === currentProject.value?.id)) {
        form.rotate = 1
    }
})
...
const submitForm = async () => {
    ...
    if (addProjectPopType.value === 'edit') {
        ...
        // 如果项目绕点旋转已存在，则更新
        const index = projectRotate.findIndex((item: any) => item.id === currentProject.value?.id)
        if (index !== -1) {
            projectRotate[index].rotate = form.rotate
        } else {
            projectRotate.push({
        ...
        localStorage.setItem('projectRotate', JSON.stringify(projectRotate))
