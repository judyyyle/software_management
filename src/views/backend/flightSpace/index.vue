<template>
    <div class="flight-space">
        ...
        <!-- 左侧选择卡 -->
        <transition name="fade">
            <LeftTabs v-if="['', 'look'].includes(openShipLanesOptionsPop) && isShowLeft" ref="leftTabs" />
        </transition>
        ...
        <!-- 右侧媒体库 -->
        <transition name="fade">
            <RightMediaLibrary v-if="activeTab === 4" />
        </transition>

        <!-- 右侧任务 -->
        <transition name="fade">
            <RightAssignment v-if="activeTab === 3 && openAssignmentPop" />
        </transition>
...
import { provide, ref, watch, reactive, computed, onMounted, onUnmounted, nextTick } from 'vue'
...
const { isShowProject, addProjectPop, currentProject, aerodromeList } = storeToRefs(projectStore)
...
const { graphicLayer_t, graphicLayer_d, isShowCabinLive, isShowDroneLive, rotatePoint } = storeToRefs(mapStore)
...
const marsOnload = (map: mars3d.Map) => {
    ...
    rotatePoint.value = new mars3d.thing.RotatePoint({
        ...
        time: 20, // 给定飞行一周所需时间(单位 秒)，控制速度
        autoStopAngle: 359, // 自动停止角度(单位 度)，默认360度
    })
    marsMap.value.addThing(rotatePoint.value)
    // 键盘漫游
    keyboardRoam.value = new mars3d.thing.KeyboardRoam({
        moveStep: 10, // 平移步长 (米)。
    ...
    marsMap.value.addThing(keyboardRoam.value)
    marsMap.value.viewer.cesiumWidget.canvas.focus()
}
...
const { isExecutingTask, deviceOsds, droneOsds, executingDroneCount, executingDeviceSn, gateway_sn } = storeToRefs(mqttStore)

watch(aerodromeSetting, (newVal: boolean) => {
    if (newVal) {
...
// 监听正在执行的无人机数量
watch(executingDroneCount, (newVal: number) => {
    if (executingDroneCount.value > 0 && !aerodromeSetting.value) {
        showElNotification()
    }
})
...
const showElNotification = () => {
    if (executingDroneCount.value > 0 && !aerodromeSetting.value) {
        ...
        ElNotification({
            ...
            duration: 0,
            onClick: () => {
                console.log('正在执行的设备', executingDeviceSn.value)
                console.log('设备列表', aerodromeList.value)
                const item = aerodromeList.value.find((item: any) => item.sn == executingDeviceSn.value[0]) || {}
...
const handleKeyDown = (e: KeyboardEvent) => {
    if (e.key === 'F2') {
        const point = marsMap.value.getCenter()
        // 开启旋转
        rotatePoint.value?.start(point)
    }
    if (e.key === 'F3') {
        // 关闭旋转
