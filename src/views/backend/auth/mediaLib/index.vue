<template>
    <div class="media-lib-management">
        <el-form ref="formRef" :model="mediaConfig" label-position="top" class="media-form">
            ...
            <!-- 直播切换 -->
            <el-form-item prop="videoType" label="直播切换">
                <div class="form-row">
                    <CustomSelect v-model="mediaConfig.video_type" placeholder="选择直播切换" :options="videoTypeOptions" width="480px" />
                </div>
            </el-form-item>
        </el-form>
...
const getMediaConfig = async () => {
    ...
    // 将obj赋值给mediaConfig
    setMediaConfig(obj)
    mediaConfig.value.video_type = mediaStore.video_type
}
...
const handleSubmit = async () => {
    ...
    try {
        ...
        // 保存直播切换类型
        mediaStore.changeVideoType(mediaConfig.value.video_type)
