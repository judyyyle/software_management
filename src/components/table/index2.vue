<template>
    <div class="ba-table-box">
                ...
                <template v-if="item.show !== false">
                    ...
                    >
                        ...
                        <template #default="scope">
                            <div style="padding-bottom: 20px">{{ getText(scope.row, item.prop) }}</div>
                            <div v-if="scope.row.children?.length">
                                <div v-for="child in scope.row.children" :key="child.id">
                                    <div style="display: flex">
                                        <img
                                            v-if="item.prop === 'model'"
                                            style="width: 14px; height: 14px; margin-right: 5px"
                                            src="/src/assets/bk.png"
                                            alt=""
                                        />
                                        <div>{{ getText1(child, item.prop) }}</div>
                                    </div>
                                </div>
                            </div>
                        </template>
        ...
        <div v-if="props.pagination" class="pagination-container">
            <div class="pagination-info">
                共 <span class="total-count">{{ baTable.table.total }}</span> 条记录
            </div>
            <el-pagination
...
import { modelCode, workCode } from '/@/utils/statusCode'
...
const getText = (row: any, prop: string) => {
    // if (prop === 'model') {
    //     return modelCode[row[prop]]
    // }
    return row[prop]
}

const getText1 = (row: any, prop: string) => {
    // if (prop === 'model') {
    //     return modelCode[row[prop]]
    // }
    return row[prop]
