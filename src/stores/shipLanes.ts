...
export const useShipLanes = defineStore('shipLanes', {
    ...
    actions: {
        ...
        showShipForm(kmzJson: any) {
            ...
            console.log('航线信息', this.shipForm)
            this.placemarkData = kmzJson.Folder.Placemark
            this.shipStartPoint = kmzJson.shipStartPoint
            this.shipLanes = kmzJson.shipLanes
            console.log('起飞点', this.shipStartPoint)
            this.placemarkData.forEach((item, index) => {
                // 相对起飞点高度
                if (this.shipForm.Folder.waylineCoordinateSysParam.heightMode === 'relativeToStartPoint') {
                    item.executeHeight = item.executeHeight - parseFloat(this.shipStartPoint[0][2])
                }
            })
            console.log('航点高度', this.placemarkData)
        },
        // 添加航点
        addPlacemark(coordinates: string, isStartPoint: boolean, alt: number) {
            if (isStartPoint) {
                const arr = this.shipStartPoint[0]
                const arr1 = [arr[1], arr[0], arr[2]]
                this.shipForm.missionConfig.takeOffRefPoint = arr1.join(',')
            } else {
                this.placemark.executeHeight = this.shipForm.Folder.globalHeight
        ...
        deletePlacemark(index: number) {
            ...
            console.log('删除后', this.placemarkData)
            this.resetId()
        },
        // 重新计算id
        resetId() {
        ...
        // 保存
        async savePlacemark() {
            // 把当前航点速度赋值给waypointSpeed
            this.placemarkData.forEach((item) => {
                item.waypointSpeed = this.shipForm.Folder.autoFlightSpeed
            })
            this.placemarkData.forEach((item) => {
                if (this.shipForm.Folder.waylineCoordinateSysParam.heightMode === 'EGM96') {
                    item.executeHeight = this.shipForm.Folder.globalHeight
                } else {
                    item.executeHeight = item.executeHeight + Number(this.shipStartPoint[0][2])
                }
            })
            ...
            //         item.executeHeight = this.shipForm.Folder.globalHeight
            //     } else {
            //         item.executeHeight = this.shipForm.Folder.globalHeight + useMqttStore().deviceData.height
            //     }
            // })
        ...
        changeHeightModel() {
            this.graphicList.forEach((item: any) => {
                // 航点模型 type=1 无人机 type=5 四凌锥
                if (item.attr.type === 1 || item.attr.type === 4 || item.attr.type === 5) {
    ...
    getters: {
        newShipLanes: (state) => {
            ...
            const arr = state.shipLanes.map((item, index) => {
                console.log(state.shipForm.Folder.waylineCoordinateSysParam.heightMode)
                console.log(state.placemarkData[index]?.executeHeight, state.shipForm.Folder.globalHeight, parseFloat(state.shipStartPoint[0][2]))
                if (state.shipForm.Folder.waylineCoordinateSysParam.heightMode === 'EGM96') {
                    return [item[0], item[1], state.placemarkData[index]?.executeHeight || state.shipForm.Folder.globalHeight]
                }
                if (state.shipForm.Folder.waylineCoordinateSysParam.heightMode === 'relativeToStartPoint') {
                    return [
                        item[0],
                        item[1],
                        (state.placemarkData[index]?.executeHeight || state.shipForm.Folder.globalHeight) + parseFloat(state.shipStartPoint[0][2]),
                    ]
                }
        ...
        newStartPoint(state) {
            ...
            state.shipForm.missionConfig.takeOffSecurityHeight = Number(state.shipForm.missionConfig.takeOffSecurityHeight)
            console.log('起飞点', this.newShipLanes[0]?.[2], state.shipForm.Folder.globalHeight, state.shipStartPoint[0][2])
            return state.shipStartPoint.map((item) => {
                if (state.shipForm.missionConfig.flyToWaylineMode === 'safely') {
                    ...
                    if (state.shipForm.Folder.waylineCoordinateSysParam.heightMode === 'relativeToStartPoint') {
                        return [item[0], item[1], this.newShipLanes[0]?.[2] || state.shipForm.Folder.globalHeight + parseFloat(item[2])]
                    }
                    if (state.shipForm.Folder.waylineCoordinateSysParam.heightMode === 'aboveGroundLevel') {
                        return [item[0], item[1], this.newShipLanes[0]?.[2] || state.shipForm.Folder.globalHeight + parseFloat(item[2])]
                    }
                } else if (state.shipForm.missionConfig.flyToWaylineMode === 'pointToPoint') {
        ...
        getExecutionTime: (state) => {
            if (state.shipForm.Folder.autoFlightSpeed === 0) return '0分0秒'
            const minutes = Math.floor(state.subForm.mileage / state.shipForm.Folder.autoFlightSpeed / 60)
            const remainingSeconds = (state.subForm.mileage / state.shipForm.Folder.autoFlightSpeed) % 60
