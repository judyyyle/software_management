...
# ── Origin 白名单（仅限本地 Vite dev server：localhost/127.0.0.1 端口 5170-5179）
...
def register_route(sock) -> None:  # type: ignore[no-untyped-def]
    ...
    def telemetry_ws(ws):  # type: ignore[no-untyped-def]
        ...
        with _conn_lock:
            _connections.add(ws)
        logger.info("[telemetry_ws] 客户端接入，当前连接数: %d", len(_connections))

        try:
            # 建连后立即推送当前完整快照
            if _snapshot_builder is not None:
                try:
                    ws.send(json.dumps(_snapshot_builder()))
